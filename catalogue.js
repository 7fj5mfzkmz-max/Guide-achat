(function () {
  ...
  var products = [];
  var filterBox = document.getElementById("catalogue-filters");
  var filterInputs = filterBox ? filterBox.querySelectorAll("[data-filter]") : [];
  ...
  function matches(p, query) {
    ...
    var haystack = [p.nom, p.marque, p.critere_principal].concat(p.profil_adapte || []).concat(Object.values(p.caracteristiques || {})).join(" ").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
    var aliases = { photo: ["photo", "camera", "appareil", "zoom", "teleobjectif"], autonomie: ["autonomie", "batterie", "mah"], performance: ["performance", "processeur", "puce", "gaming", "jeu"], ecran: ["ecran", "oled", "amoled", "hz", "luminosite"], stockage: ["stockage", "go", "ufs"], charge: ["charge", "w", "recharge"], compact: ["compact", "petit", "taille"] };
    var terms = q.split(/\s+/).filter(Boolean);
    return terms.every(function(term) {
      if (haystack.indexOf(term) !== -1) return true;
      return Object.keys(aliases).some(function(key) { return aliases[key].indexOf(term) !== -1 && aliases[key].some(function(alias) { return haystack.indexOf(alias) !== -1; }); });
    });
  }

  function activeFilters() {
    var filters = { price: "all", priority: "all", storage: "all", screen: "all", telephoto: "all", charge: "all" };
    filterInputs.forEach(function(input) { filters[input.dataset.filter] = input.value; });
    return filters;
  }

  function matchesFilters(p, filters) {
    var text = textOf(p);
    if (filters.price === "moins-300" && !(typeof p.prix_indicatif === "number" && p.prix_indicatif < 300)) return false;
    if (filters.price === "300-700" && !(typeof p.prix_indicatif === "number" && p.prix_indicatif >= 300 && p.prix_indicatif < 700)) return false;
    if (filters.price === "700-plus" && !(typeof p.prix_indicatif === "number" && p.prix_indicatif >= 700)) return false;
    if (filters.priority !== "all" && !(p.profil_adapte || []).some(function(x) { return x.toLowerCase().indexOf(filters.priority) !== -1; }) && !(p.critere_principal || "").toLowerCase().indexOf(filters.priority) !== -1) return false;
    if (filters.storage !== "all") { var n = parseInt(filters.storage, 10); if (!new RegExp("(?:" + n + "|" + (n >= 256 ? "512" : "256|512") + ")\\s*go", "i").test(text)) return false; }
    if (filters.screen === "oled" && !/(oled|amoled|super amoled|retina)/i.test(text)) return false;
    if (filters.screen === "120hz" && !/120\s*hz/i.test(text)) return false;
    if (filters.telephoto === "yes" && !/(teleobjectif|téléobjectif|zoom optique)/i.test(text)) return false;
    if (filters.charge === "fast" && !/(45|50|60|65|67|80|90|100|120)\s*w/i.test(text)) return false;
    return true;
  }
  ...
  function render() {
    ...
    var visible = products.filter(function (p) { return matches(p, query) && matchesFilters(p, filters); });
  ...
  filterInputs.forEach(function(input) { input.addEventListener("change", render); });
})();
